...
if(!core||!data||!evidence)return{ok:false,errors:['evidence_missing']};var s=data.source||{},p=data.period||{},at=now==null?Date.now():now,required=core.requiredCount?core.requiredCount(data):20;
need(data.schema_version===1&&data.status==='verified'&&data.validation&&data.validation.verified===true,'public_status');
need(Number.isSafeInteger(required)&&required>=1&&required<=100,'required_count');
...
need(Array.isArray(evidence.models)&&evidence.models.length>=required&&evidence.models.length<=10000&&Array.isArray(evidence.results)&&evidence.results.length<=10000,'evidence_inventory');
...
if(errors.length)return{ok:false,errors:errors};var rebuilt=core.project(evidence.models,{results:evidence.results},{collected_at:evidence.collected_at,geo_target:evidence.geo_target,response_sha256:s.evidence_sha256,required_count:required,title:data.title,category:data.category}),checked=core.validate(rebuilt,at);need(checked.verified,'upstream_metrics_invalid');if(!checked.verified)return{ok:false,errors:errors.concat(checked.errors)};
var end=rebuilt.period.end,lastDay=new Date(Date.UTC(Number(end.slice(0,4)),Number(end.slice(5)),0)).toISOString().slice(0,10);need(p.start===rebuilt.period.start+'-01'&&p.end===lastDay&&p.label===p.start+' — '+p.end,'period_mismatch');need(Array.isArray(data.rows)&&stable(rowView(data.rows))===stable(rowView(rebuilt.rows)),'measurements_mismatch');need(stable(data.coverage)===stable(rebuilt.coverage),'coverage_mismatch');var lastChecked=Date.parse(data.last_checked),collected=Date.parse(data.last_updated);need(Number.isFinite(lastChecked)&&lastChecked>=collected&&lastChecked<=at,'check_date');return{ok:errors.length===0,errors:errors,measured_models:rebuilt.rows.length,required_count:required}
