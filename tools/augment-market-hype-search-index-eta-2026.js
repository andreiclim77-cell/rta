...
const CFG='data/market-hype-sources-2026.json',P='data/market-hype-products-2026.json';
...
function canonical(s){let x=clean(s).replace(/^Buy\s+/i,'').replace(/\s*\|\s*.*$/,'');if(x.includes(' - '))x=x.split(' - ')[0];return x.trim().slice(0,190)}
...
function brand(name,cfg){const t=norm(name),seen=new Set();for(const x of cfg.cloneMakers||[]){const k=norm(x);if(!k||seen.has(k))continue;seen.add(k);if(t.includes(k))return String(x).toUpperCase()}for(const m of cfg.officialMakers||[])for(const a of String(m.name||'').split(/[\/|]/)){const k=norm(a);if(k.length>=3&&t.includes(k))return m.name}return clean(name).split(/\s+/)[0]||null}
...
async function main(){
 const cfg=read(CFG,{}),old=read(P,{products:[]}),map=new Map();
 ...
 for(const d of domains){queries.push({query:`site:${d} RTA "next batch" 2026`});queries.push({query:`site:${d} RTA "shipped around" 2026`});queries.push({query:`site:${d} RTA "product is delayed" 2026`});queries.push({query:`site:${d} RTA ETA 2026 preorder`})}
 const runs=await limit(queries,7,bing);
 const docs=[...new Map(runs.flatMap(x=>x.docs||[]).filter(x=>x.url&&allowed(x.url,cfg)&&rta(x.title+' '+x.body)&&!accessory(x.title)).map(x=>[x.url.split('#')[0],x])).values()].slice(0,220);
 const pages=await limit(docs,10,async x=>{try{const f=await get(x.url);return{x,url:f.url,html:f.text,error:null}}catch(e){return{x,url:x.url,html:'',error:String(e&&e.message||e)}}});
 ...
 for(const z of pages){
   ...
   events++;if(sample.length<24)sample.push({name,eventDate:ev.eventDate,stage:ev.stageLabel,url:u,evidenceMode});
 }
 const products=[...map.values()].filter(x=>x&&x.productName&&x.eventDate),doc={...old,schemaVersion:Math.max(22,Number(old.schemaVersion||0)),generatedAt:now,pendingRefresh:false,truth:{...(old.truth||{}),searchIndexVendorEtaRecovery:true,searchIndexSnippetFallback:true,searchIndexSnippetRequiresExplicitDate:true,searchIndexSnippetDiscoveryOnly:true,pastEtaIsNotRelease:true,symmetric30DayEventWindow:true},scan:{...(old.scan||{}),searchIndexEtaQueries:queries.length,searchIndexEtaWorking:runs.filter(x=>x.ok).length,searchIndexEtaCandidates:docs.length,searchIndexEtaPages:pages.filter(x=>x&&!x.error&&x.html).length,searchIndexEtaPhraseHits:phraseHits,searchIndexEtaSnippetFallbacks:snippetFallbacks,searchIndexEtaEvents:events,searchIndexEtaSample:sample},products,summary:{...(old.summary||{}),total:products.length,before:products.filter(x=>x.window==='before').length,after:products.filter(x=>x.window==='after').length}};
 if(WRITE)save(P,doc);
 console.log(`Hype search-index ETA: ${runs.filter(x=>x.ok).length}/${queries.length} queries; candidates ${docs.length}; pages ${doc.scan.searchIndexEtaPages}; snippet fallbacks ${snippetFallbacks}; phrase hits ${phraseHits}; events ${events}; products ${products.length}.`);
 if(sample.length)console.log('Hype search-index ETA events:',sample.map(x=>`${x.name} | ${x.stage} | ${x.eventDate} | ${x.evidenceMode}`).join(' || '));
}
