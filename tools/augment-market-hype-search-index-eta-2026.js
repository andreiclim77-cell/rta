...
async function main(){
 ...
 const highYieldVendorDomains=['3fvape.com','2fdeal.com'].filter(d=>domains.includes(d));
 const makerGroups=[];
 ...
 for(const m of makerGroups){const mk=norm(m.name);if(!mk||seenMakers.has(mk))continue;seenMakers.add(mk);const clause=makerClause(m.terms);if(!clause)continue;queries.push({query:`${clause} RTA "next batch" 2026`,mode:'maker-targeted',maker:m.name});queries.push({query:`${clause} RTA "product is delayed" 2026`,mode:'maker-targeted',maker:m.name});for(const d of highYieldVendorDomains)queries.push({query:`site:${d} ${clause} RTA 2026 preorder`,mode:'maker-vendor-targeted',maker:m.name,vendorDomain:d})}
 ...
 const runs=await limit(uniqueQueries,10,bing);
 const docs=[...new Map(runs.flatMap(x=>x.docs||[]).filter(x=>x.url&&allowed(x.url,cfg)&&rta(x.title+' '+x.body)&&!accessory(x.title)).map(x=>[x.url.split('#')[0],x])).values()].slice(0,460);
 const pages=await limit(docs.slice(0,320),12,async x=>{try{const f=await get(x.url);return{x,url:f.url,html:f.text,error:null}}catch(e){return{x,url:x.url,html:'',error:String(e&&e.message||e)}}});
 ...
 for(const z of pages){
   ...
   events++;if(sample.length<60)sample.push({name,eventDate:ev.eventDate,stage:ev.stageLabel,url:u,evidenceMode,queryMode:z.x.mode||null,queryMaker:z.x.maker||null,vendorDomain:z.x.vendorDomain||null});
 }
 const products=[...map.values()].filter(x=>x&&x.productName&&x.eventDate),doc={...old,schemaVersion:Math.max(27,Number(old.schemaVersion||0)),generatedAt:now,pendingRefresh:false,truth:{...(old.truth||{}),searchIndexVendorEtaRecovery:true,searchIndexMakerTargeting:true,searchIndexCloneMakerTargeting:true,searchIndexMakerVendorTargeting:true,searchIndexSnippetFallback:true,searchIndexSnippetRequiresExplicitDate:true,searchIndexSnippetDiscoveryOnly:true,pastEtaIsNotRelease:true,symmetric30DayEventWindow:true},scan:{...(old.scan||{}),searchIndexEtaQueries:uniqueQueries.length,searchIndexEtaWorking:runs.filter(x=>x.ok).length,searchIndexEtaMakerTargets:seenMakers.size,searchIndexEtaCloneMakerRegistry:(cfg.cloneMakers||[]).length,searchIndexEtaMakerVendorDomains:highYieldVendorDomains,searchIndexEtaCandidates:docs.length,searchIndexEtaPages:pages.filter(x=>x&&!x.error&&x.html).length,searchIndexEtaPhraseHits:phraseHits,searchIndexEtaSnippetFallbacks:snippetFallbacks,searchIndexEtaEvents:events,searchIndexEtaSample:sample},products,summary:{...(old.summary||{}),total:products.length,before:products.filter(x=>x.window==='before').length,after:products.filter(x=>x.window==='after').length}};
 if(WRITE)save(P,doc);
 console.log(`Hype search-index ETA: ${runs.filter(x=>x.ok).length}/${uniqueQueries.length} queries; maker targets ${seenMakers.size}; clone registry ${(cfg.cloneMakers||[]).length}; maker-vendor domains ${highYieldVendorDomains.length}; candidates ${docs.length}; pages ${doc.scan.searchIndexEtaPages}; snippet fallbacks ${snippetFallbacks}; phrase hits ${phraseHits}; events ${events}; products ${products.length}.`);
 if(sample.length)console.log('Hype search-index ETA events:',sample.map(x=>`${x.name} | ${x.stage} | ${x.eventDate} | ${x.evidenceMode} | ${x.queryMaker||x.queryMode||''} | ${x.vendorDomain||''}`).join(' || '));
}
