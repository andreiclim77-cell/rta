...
for(const asset of ['/assets/market-loading-guard.js?v=15','/assets/market-ui-recovery.js?v=9','/assets/market-hype-ui.js?v=9','/assets/market-view-switcher.js?v=7','/assets/market-access-policy.js?v=2'])need(enhancements.includes(asset),`Loader is not on final asset ${asset}`);
for(const asset of ['/assets/market-loading-guard.js?v=15','/assets/market-ui-recovery.js?v=9','/assets/market-hype-ui.js?v=9','/assets/market-hype-ui.css?v=7','/assets/market-view-switcher.js?v=7','/assets/market-source-info.js?v=3','/assets/market-access-policy.js?v=2'])need(sw.includes(asset),`Service worker is not on final asset ${asset}`);
need(sw.includes('ghid-rta-static-v39-hype-live-progress'),'Final service worker cache version missing');
...
need(recovery.includes('market-hype-ui.js?v=9'),'Recovery can inject old Hype UI');
...
need(hype.includes('availabilityBlock')&&hype.includes('Disponibilitate și listări observate')&&hype.includes('LISTARE OBSERVATĂ'),'Current availability and commercial listings are not separated in the Hype UI');
...
need(hype.includes('Memoria de 180 de zile împiedică relistările vechi')&&hype.includes('Lansările datate, disponibilitatea curentă'),'UI does not explain relisting/release/availability distinction');
...
console.log('Market final gate OK: Analysis=Romania; Hype=GLOBAL exact ±30d, RTA/MOD/ACCESSORIES+POD, dated events separated from current availability and public coverage, source arbitration and canonical deduplication enforced, 06:00 primary + 06:20 recovery; password every entry; cache v39.');
