...
for(const asset of ['/assets/market-loading-guard.js?v=14','/assets/market-ui-recovery.js?v=7','/assets/market-hype-ui.js?v=6','/assets/market-view-switcher.js?v=7','/assets/market-access-policy.js?v=2'])need(enhancements.includes(asset),`Loader is not on final asset ${asset}`);
for(const asset of ['/assets/market-loading-guard.js?v=14','/assets/market-ui-recovery.js?v=7','/assets/market-hype-ui.js?v=6','/assets/market-hype-ui.css?v=4','/assets/market-view-switcher.js?v=7','/assets/market-source-info.js?v=3','/assets/market-access-policy.js?v=2'])need(sw.includes(asset),`Service worker is not on final asset ${asset}`);
need(sw.includes('ghid-rta-static-v36-hype-product-level'),'Final service worker cache version missing');
need(sw.includes('/data/market-hype-products-2026.json'),'Product-level Hype data are not network-first');
need(recovery.includes('market-hype-ui.js?v=6'),'Recovery can inject old Hype UI');
need(guard.includes("key:'hypeProducts'")&&guard.includes("afterReleaseRequiresExplicitDate")&&guard.includes("dateConfidence==='explicit'"),'Atomic guard does not validate concrete Hype products and dated releases');
...
for(const token of ['productLevelOnly:true','newArrivalIsNotRelease:true','afterReleaseRequiresExplicitDate:true','relistingIsNotRelease:true','BANNED_HOSTS','market-hype-products-2026.json'])need(products.includes(token),`Product-level Hype protection missing: ${token}`);
...
need(hypeFlow.includes('collect-market-hype-products-2026.js --write'),'Product-level Hype is not in 06:00 pipeline');
...
console.log('Market final gate OK: Analysis=Romania; Hype=GLOBAL 30d, product-level, expandable, dated after-release only; password every entry; cache v36.');
