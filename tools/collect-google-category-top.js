...
if(!cfg){console.error('Use --kind=rta or --kind=mod');process.exit(2)}
...
function candidates(demand,canonicalize,isUsable){
  ...
  for(const p of demand.products||[]){
    if(p.category!==cfg.category||p.inRomanianMarket!==true||!isUsable(p))continue;
    const c=canonicalize(p);
    if(!c.brand||!c.model||!core.norm(c.model)||blockedRaw(c.rawProduct,cfg.kind))continue;
...
function toPublic(internal,evidenceSha){
  ...
  out.source.evidence_file='/'+cfg.evidenceDir+'/api-'+out.last_checked.slice(0,10)+'-'+evidenceSha.slice(0,16)+'.json';
...
function report(out){
  const summary={kind:cfg.kind,status:out.status,google_rows:out.rows.length,required_count:cfg.required,models_requested:out.coverage&&out.coverage.models_requested||0,models_with_valid_series:out.coverage&&out.coverage.models_with_valid_series||0,errors:out.validation&&out.validation.errors||[]};
  if(out.diagnostic)summary.diagnostic=out.diagnostic;console.log(JSON.stringify(summary));
  if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,'## Google '+cfg.title+'\n\n```json\n'+JSON.stringify(summary,null,2)+'\n```\n');
}
async function main(){
  const missing=REQUIRED.filter(k=>!String(process.env[k]||'').trim());
  if(missing.length){const out=unavailable('configuration_required','Conexiunea Google Ads nu este configurată complet. Nu publicăm valori de rezervă.',{missing_secret_names:missing});save(cfg.out,out);report(out);return;}
  const progress={kind:cfg.kind,phase:'candidate_inventory',oauth_authenticated:false,geography_resolved:false,models_requested:0};
  try{
    const {canonicalizeProduct,isUsableProductTitle}=require('./market-product-canonical-2026');
    const demand=load('data/market-demand-intelligence-2026.json'),models=candidates(demand,canonicalizeProduct,isUsableProductTitle);progress.models_requested=models.length;
    if(models.length<cfg.required||models.length>10000)throw new Error('Candidate inventory outside allowed range');
    ...
    const evidence={schema_version:1,collected_at,api_version:version,country:'RO',category:cfg.category,required_count:cfg.required,network:'GOOGLE_SEARCH',geo_target:country.resourceName,language_filter:'not_set',include_adult_keywords:true,models,results:response.results.map(r=>({text:r.text,closeVariants:r.closeVariants||[],keywordMetrics:{avgMonthlySearches:r.keywordMetrics&&r.keywordMetrics.avgMonthlySearches,monthlySearchVolumes:r.keywordMetrics&&r.keywordMetrics.monthlySearchVolumes||[]}}))};
    const evidenceText=JSON.stringify(evidence,null,2)+'\n',evidenceSha=sha(evidenceText);
    const internal=core.project(models,{results:evidence.results},{collected_at,geo_target:country.resourceName,response_sha256:evidenceSha,required_count:cfg.required,title:cfg.title,category:cfg.category});
    const out=toPublic(internal,evidenceSha);fs.mkdirSync(cfg.evidenceDir,{recursive:true});fs.writeFileSync(out.source.evidence_file.slice(1),evidenceText);
    ...
    const out=unavailable('source_error','Colectarea Google a eșuat. Nu publicăm un clasament de rezervă.',{error:safe,...progress,...(e.googleDiagnostic||{})});out.coverage.models_requested=progress.models_requested;save(cfg.out,out);report(out);process.exitCode=1;
  }
}
module.exports={CONFIG,candidates,toPublic,safeGoogleError};
if(require.main===module)main().catch(()=>{console.error('Google category collector failed before collection');process.exitCode=1;});
