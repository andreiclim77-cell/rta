...
// Return only structured error identifiers. Never expose response messages,
// request bodies, headers, tokens, customer IDs, or error trigger values.
function safeGoogleError(payload,status){
  ...
  if(error&&typeof error==='object'){
    ...
    for(const detail of Array.isArray(error.details)?error.details:[]){
      for(const item of detail&&Array.isArray(detail.errors)?detail.errors:[]){
        for(const [kind,code] of Object.entries(item&&item.errorCode||{})){
          if(safe(kind,/^[a-z][a-zA-Z]{1,80}Error$/)&&safe(code,/^[A-Z][A-Z0-9_]{1,100}$/))codes.push(kind+'.'+code);
        }
      }
    }
...
async function request(url,body,headers={},timeout=30000){
  ...
  if(!response.ok){
    ...
    error.googleDiagnostic=safeGoogleError(payload,response.status);
    throw error;
  }
...
function report(out){
  ...
  if(out.diagnostic)summary.diagnostic=out.diagnostic;
...
async function main(){
  ...
  const progress={phase:'candidate_inventory',oauth_authenticated:false,geography_resolved:false,models_requested:0};
  try{
    ...
    progress.oauth_authenticated=true;
    ...
    progress.phase='geography_lookup';
    ...
    progress.geography_resolved=true;
    const requestBody={keywords:models.map(x=>x.query),geoTargetConstants:[country.resourceName],keywordPlanNetwork:'GOOGLE_SEARCH',includeAdultKeywords:true};
    ...
    progress.phase='evidence_validation';
    ...
    out.source.include_adult_keywords=true;
    ...
    const safeMessages=new Set(['Invalid Google Ads customer ID format','Invalid Google Ads API version','Google OAuth did not return an access token','Romania country target could not be resolved','Google returned no historical metric results','Candidate inventory must contain 20 to 10000 canonical models']);
    ...
    const details={error:safe,...progress,...(e.googleDiagnostic||{})};
    const out=unavailable('source_error','Colectarea Google a eșuat. Nu publicăm un clasament de rezervă.',details);
    out.coverage.models_requested=progress.models_requested;
    save(OUT,out);report(out);process.exitCode=1;
  }
}
module.exports={candidates,unavailable,toPublic,safeGoogleError};
