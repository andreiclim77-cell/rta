...
async function bing(meta){try{const result=await fetchText('https://www.bing.com/search?format=rss&q='+encodeURIComponent(meta.query),10000);return{ok:true,docs:rssRows(result.text,meta)}}catch(error){return{ok:false,docs:[],error:String(error&&error.message||error)}}}
...
function canonicalTitle(value){return decode(value).replace(/^Buy\s+/i,'').replace(/\s*[|–—]\s*(?:official|review|preview|youtube|shop|store).*$/i,'').replace(/\s+/g,' ').trim().slice(0,190)}
...
function genericPodPage(title,url){const value=decode(title),t=norm(value),path=String(url||'').toLowerCase();return /\b(?:complete guide|all products|product range|vape series|open pod systems|pod kits for sale|vapes in|vape price|buy .* online|meaning of|definition|login|portal|produse veev|tigari electronice veev|hayati vapes)\b/i.test(value)||/\/(?:collections?|pages\/products|brands?|wiki)(?:\/|$)/.test(path)||/\/(?:c|category)(?:\/|$)/.test(path)||/^(?:uwell )?caliburn(?: myuwell com)?$/.test(t)||/\bxros\b.*\bluxe\b/.test(t)}
...
async function main(){
  ...
  const output={schemaVersion:4,scopeYear:2026,scope:'GLOBAL POD SYSTEMS',windowDays:30,discoveryContextDays:180,generatedAt:now,snapshotReferenceAt:new Date(REF).toISOString(),dailyWindowTimezone:'Europe/Bucharest',pendingRefresh:false,truth:{registryIsDiscoveryOnly:true,concreteProductRequired:true,explicitOrDatedEventRequired:true,vendorRelistingIsNotRelease:true,rejectedCandidatesAreNotPublishedAsLaunches:true,beforeAndAfterWindowsAreThirtyDays:true,publicOfficialSocialForumNewsRetailDiscovery:true,retailFirstSeenIsHandledOnlyByCatalogObserver:true,retailMemoryContextDays:180},scan:{makersConfigured:(reg.makers||[]).length,segmentsConfigured:(reg.segments||[]).length,queriesRun:queries.length,queriesWorking:working,searchDocuments:allDocs.length,candidateDocuments:docs.length,pagesProcessed:pages.length,concreteProducts:concrete,retailProductsObserved:retailObserved,firstRetailEvents:0,verifiedSignals:products.length,rejections,candidatesUnderVerification:queue.length,rejectedSample:queue.slice(0,40)},verificationQueue:queue,products,summary:{total:products.length,before:products.filter(function(product){return product.window==='before'}).length,after:products.filter(function(product){return product.window==='after'}).length,candidatesUnderVerification:queue.length}};
