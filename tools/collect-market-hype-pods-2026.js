...
function falsePodCandidate(title,url){return/\b(?:veeva systems|stock price|stock quote|company profile|google finance|yahoo finance|cnn markets?|lennar|follow(?:er|ers)|discontinued|vape pod systems?\s*&\s*disposables|vape tanks?,?\s*pod mod kits?\s*&\s*accessories|smoktech mods?,?\s*pod\s*&\s*starter kits?|smoktech pens?,?\s*mods?,?\s*and pod systems?|keep it real)\b/i.test(decode(title))||/(?:finance\.yahoo|google\.[^/]+\/finance|cnn\.com\/markets)/i.test(String(url||''))}
...
function selectCandidatePages(allDocs){const groups=new Map();for(const doc of allDocs){if(!makerHit(doc.maker,doc.title+' '+doc.description)&&!(doc.maker.domains||[]).some(function(domain){return domainMatch(host(doc.url),domain)}))continue;const key=norm(doc.maker.name)+'|'+String(doc.channel||'open-web'),rows=groups.get(key)||[];rows.push(doc);groups.set(key,rows)}const selected=[];for(const [key,rows] of groups){const channel=key.split('|').pop(),budget=/official|facebook|instagram|social-video/.test(channel)?5:4;selected.push(...rows.slice(0,budget))}return selected}
function mergeVerificationQueue(previous,fresh,products,now){const published=new Set((products||[]).map(function(row){return canonicalProductFamily(row).key})),map=new Map(),maxAge=180*86400000;function add(input,isFresh){if(!input)return;const productName=input.productName||input.product||'';if(!productName)return;const key=input.familyKey||canonicalProductFamily({productName,brand:input.brand||'',category:'POD'}).key;if(!key||published.has(key))return;const old=map.get(key),first=input.firstObservedAt||input.observedAt||input.publishedAt||now,last=isFresh?now:(input.lastObservedAt||input.observedAt||input.publishedAt||first);if(Date.parse(last)&&Date.parse(now)-Date.parse(last)>maxAge)return;const sources=unique([...(old&&old.sources||[]),...(input.sources||[]),input.url].filter(Boolean));map.set(key,{...(old||{}),...input,productName,familyKey:key,reason:input.reason==='noDateOrEvent'?'undatedPublicAnnouncement':input.reason||'underVerification',firstObservedAt:old&&old.firstObservedAt||first,lastObservedAt:last,observedAt:first,sources,url:input.url||old&&old.url||sources[0]||null,status:'NEEDS_DATE_OR_ETA'})}for(const candidate of previous||[])add(candidate,false);for(const candidate of fresh||[])add(candidate,true);return Array.from(map.values()).sort(function(a,b){return String(b.lastObservedAt||'').localeCompare(String(a.lastObservedAt||''))})}
...
async function main(){
  ...
  for(const maker of reg.makers||[]){
    ...
    queries.push({maker,channel:'facebook-public',query:`site:facebook.com (${who}) ${what} ${trigger}`});
    queries.push({maker,channel:'instagram-public',query:`site:instagram.com (${who}) ${what} ${trigger}`});
    queries.push({maker,channel:'social-video',query:`(${who}) ${what} ${trigger} (site:youtube.com OR site:x.com OR site:threads.net OR site:threads.com OR site:tiktok.com)`});
  ...
  function reject(reason,page,product,classification){rejections[reason]=(rejections[reason]||0)+1;if(reason==='outsideWindow'||!['noDateOrEvent','sourceNotDecisionEligible'].includes(reason)||!product||!classification||falsePodCandidate(product,page.resolved))return;if(reason==='noDateOrEvent'&&!undatedAnnouncement((page.doc.title||'')+' '+(page.doc.description||'')))return;candidates.push({brand:classification.brand||page.doc.maker.name,productName:product,category:'POD',segment:classification.segment,typology:classification.typology,reason,channel:page.doc.channel||'open-web',url:page.resolved,publishedAt:page.doc.publishedAt||pageDate(page.html)||null,observedAt:now})}
  for(const page of pages){
    ...
    if(!classification||classification.confidence==='generic-pod-device'||genericPodPage(productName,page.resolved)||falsePodCandidate(productName,page.resolved)||classification.brand&&norm(classification.brand)!==norm(maker.name)&&!makerHit(maker,productName)){reject('notConcretePodProduct',page,productName,classification);continue}
    concrete++;
  ...
  const products=Array.from(map.values()).sort(function(a,b){return String(a.eventDate).localeCompare(String(b.eventDate))}),queue=mergeVerificationQueue(old.verificationQueue||[],candidates,products,now);
  ...
  const output={schemaVersion:6,scopeYear:2026,scope:'GLOBAL POD SYSTEMS',windowDays:30,discoveryContextDays:180,generatedAt:now,snapshotReferenceAt:new Date(REF).toISOString(),dailyWindowTimezone:'Europe/Bucharest',pendingRefresh:false,truth:{registryIsDiscoveryOnly:true,concreteProductRequired:true,explicitOrDatedEventRequired:true,vendorRelistingIsNotRelease:true,rejectedCandidatesAreNotPublishedAsLaunches:true,undatedIdentifiableProductsPreserved:true,undatedCandidatesAreNotLaunches:true,facebookAndInstagramQueriedSeparately:true,globalDocumentCapReplacedByPerMakerChannelBudgets:true,beforeAndAfterWindowsAreThirtyDays:true,publicOfficialSocialForumNewsRetailDiscovery:true,retailFirstSeenIsHandledOnlyByCatalogObserver:true,retailMemoryContextDays:180,segmentCoverageIsMeasured:true},scan:{makersConfigured:(reg.makers||[]).length,segmentsConfigured:(reg.segments||[]).length,queriesRun:queries.length,queriesWorking:working,searchDocuments:allDocs.length,candidateDocuments:docs.length,pagesProcessed:pages.length,concreteProducts:concrete,retailProductsObserved:retailObserved,firstRetailEvents:0,verifiedSignals:products.length,segmentCoverage,rejections,candidatesUnderVerification:queue.length,rejectedSample:queue.slice(0,40)},verificationQueue:queue,products,summary:{total:products.length,before:products.filter(function(product){return product.window==='before'}).length,after:products.filter(function(product){return product.window==='after'}).length,candidatesUnderVerification:queue.length}};
