...
async function main(){
  const runs=[];for(const source of SOURCES)for(const page of source.pages){const url=new URL(page,source.baseUrl).toString();try{const html=await fetchText(url);runs.push({source,url,ok:true,rows:anchors(html,source,url)})}catch(error){runs.push({source,url,ok:false,rows:[],error:String(error&&error.message||error)})}}
  ...
  const counts=Array.from(grouped.values()).reduce(function(acc,row){acc[row.category]=(acc[row.category]||0)+1;return acc},{}),stats={sources:SOURCES.length,pages:runs.length,pagesWorking:runs.filter(function(run){return run.ok}).length,signals:grouped.size,RTA:counts.RTA||0,MODURI:counts.MODURI||0,POD:counts.POD||0,rtaModAdded:rtaAdded,podsAdded};finalize(rta,stats);finalize(pods,stats);
