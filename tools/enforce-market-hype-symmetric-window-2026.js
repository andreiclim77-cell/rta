...
const REF=snapshotReferenceMs(),LIMIT=720*36e5,SIGNAL_LIMIT=30*864e5,FORECAST_LIMIT=180*864e5,refIso=new Date(REF).toISOString();
...
const latest=values=>{const parsed=values.map(ms).filter(x=>x!=null&&x<=REF+864e5);return parsed.length?Math.max(...parsed):null};
const signalObservedAt=x=>latest([x.signalObservedAt,x.firstPublicEvidenceAt,x.firstSeenAt].concat((x.sources||[]).flatMap(source=>[source.publishedAt,source.observedAt])));
const forwardSignal=x=>x&&x.window==='before'&&ms(x.eventDate)>REF+LIMIT&&ms(x.eventDate)<=REF+FORECAST_LIMIT&&/(?:teaser|prototype|sample|production|preorder|pre-order|pre sale|waitlist|eta|batch|announcement|coming|imminent)/i.test([x.stage,x.stageLabel,x.signalKind,(x.sources||[]).map(source=>source.stage+' '+source.title).join(' ')].join(' '));
function preserveForwardSignal(input){
  if(!forwardSignal(input))return input;
  const observed=signalObservedAt(input);
  if(observed==null||observed<REF-SIGNAL_LIMIT)return input;
  return{...input,confidenceTier:'public-signal',signalKind:'long-range-forecast',signalWindowOnly:true,forecastEventDate:input.eventDate,signalObservedAt:new Date(observed).toISOString(),stageEvidenceAt:new Date(observed).toISOString(),dateConfidence:'forecast-eta',stageLabel:'semnal recent cu ETA viitor, în urmărire',ageHours:Number(((REF-observed)/36e5).toFixed(1))};
}
const validProduct=x=>x&&x.eventDate&&(symmetric(x.eventDate)&&(x.window!=='after'||ms(x.eventDate)<=REF)||x.signalWindowOnly===true&&signalObservedAt(x)>=REF-SIGNAL_LIMIT);
...
p.products=(p.products||[]).map(preserveForwardSignal).filter(validProduct);
...
p.truth={...(p.truth||{}),symmetric30DayEventWindow:true,signalObservationWindowDays:30,signalLookback30dEtaHorizon365d:false,signalLookback30dEtaHorizon180d:true,futureEtaMayExceed30Days:true,forecastHorizonDays:180,longRangeSignalsAreNotLaunches:true};
