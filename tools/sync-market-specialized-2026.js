...
function clean(input){return String(input||'').replace(/<script\b[\s\S]*?<\/script>/gi,' ').replace(/<style\b[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;|&#160;/gi,' ').replace(/&amp;/gi,'&').replace(/&quot;/gi,'"').replace(/&#039;|&apos;/gi,"'").replace(/\s+/g,' ').trim()}
...
function numberFromPrice(text){
  const values=(clean(text).match(/\d{1,5}(?:[ .]\d{3})*(?:[,.]\d{1,2})?/g)||[]).map(raw=>Number(raw.replace(/\s/g,'').replace(/\.(?=\d{3}(?:\D|$))/g,'').replace(',','.'))).filter(Number.isFinite);
  return values.length?values[values.length-1]:null;
}
function attr(tag,name){const m=String(tag||'').match(new RegExp(name+'=["\\\']([^"\\\']+)["\\\']','i'));return m?m[1]:''}
function firstMatch(text,patterns){for(const p of patterns){const m=String(text||'').match(p);if(m)return m[1]||''}return ''}
function titleFromBlock(block){return clean(firstMatch(block,[/<h2\b[^>]*class=["'][^"']*(?:woocommerce-loop-product__title|product-title)[^"']*["'][^>]*>([\s\S]*?)<\/h2>/i,/<h4\b[^>]*>\s*<a\b[^>]*>([\s\S]*?)<\/a>/i,/<h3\b[^>]*>\s*<a\b[^>]*>([\s\S]*?)<\/a>/i]))}
function urlFromBlock(block,base){const m=String(block||'').match(/<a\b[^>]*href=["']([^"']+)["'][^>]*>/i);return m?canonical(absolute(base,m[1])):''}
...
async function fetchText(url,userAgent){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),20000);try{const response=await fetch(url,{headers:{'user-agent':userAgent,'accept':'text/html,application/xhtml+xml;q=0.9,*/*;q=0.8','accept-language':'ro-RO,ro;q=0.9,en;q=0.7'},redirect:'follow',signal:controller.signal});if(!response.ok)throw new Error(`HTTP ${response.status}`);return {url:response.url||url,text:await response.text()}}finally{clearTimeout(timer)}}
...
async function collect(retailer,type,registry,date){
  ...
  for(const seed of retailer.seeds||[]){const queue=[seed.url];let seedOk=false;let guard=0;while(queue.length&&guard<50){guard++;const current=queue.shift();const key=canonical(current);if(visited.has(key))continue;visited.add(key);try{const fetched=await fetchText(current,userAgent);pagesFetched++;seedOk=true;rows.push(...parsePage(retailer,fetched.url,fetched.text,type,seed.categoryHint,date));for(const next of pagination(fetched.text,fetched.url))if(!visited.has(next))queue.push(next)}catch(error){errors.push({url:current,error:String(error&&error.message||error).slice(0,180)})}await sleep(650)}if(seedOk)seedsSucceeded++;else seedsFailed++}
  return {retailerId:retailer.id,parser:type,pagesFetched,seedsConfigured:(retailer.seeds||[]).length,seedsSucceeded,seedsFailed,rows:dedupe(rows),errors,collectionComplete:seedsFailed===0&&seedsSucceeded===(retailer.seeds||[]).length}
}
...
async function main(){
  ...
  const specialized={vapshop:'opencart',viciishop:'woo',tigaraego:'woo',vapersparadise:'woo',vapetronic:'woo',steamfactory:'woo'};
  const results=[];
  ...
  for(const result of results){
    if(result.rows.length){observations=observations.filter(r=>r.retailerId!==result.retailerId).concat(result.rows)}
  }
