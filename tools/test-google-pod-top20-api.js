...
invalid('overlapping close variants',d=>{d.rows[1].keyword_variants.push(d.rows[0].keyword_variants[0])});
...
test('duplicate Google groups do not double count',()=>{const r=clone(response);r.results.push(clone(r.results[0]));const d=core.project(models,r,meta);assert.equal(d.coverage.excluded_groups.overlap,2);assert.equal(d.rows.length,23)});
